-- find genera that are ECM in the top 75 species by basal area
WITH relevant_genera AS ( 
  SELECT 
    fa.genus, 
    rs.association
  FROM fire_adaptation fa
  JOIN ref_species rs
  ON fa.genus = rs.genus
  WHERE association = 'EM'
  GROUP BY 
    fa.genus,
    rs.association
)
...
-- filter the delta percent basal area by province and genus table to only include ECM genera from the top 75 species by basal area
...
WHERE 
	t1.genus IN ('Ostrya','Tilia','Quercus','Abies','Populus','Betula','Carya','Picea','Tsuga','Larix','Pinus','Fagus')
	OR t2.genus in ('Ostrya','Tilia','Quercus','Abies','Populus','Betula','Carya','Picea','Tsuga','Larix','Pinus','Fagus')
ORDER BY 
	region_id,
